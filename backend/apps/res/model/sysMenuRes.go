...
type MenuRes struct {
 ...
 func GetResByMenuId(mid int64, domain string) ([]*Resource, error) {
 	...
 	err := Orm.Table(&Resource{}).Join("INNER", "menu_res", "resource.id = menu_res.res_id").Where("menu_res.menu_id = ?", mid).And("menu_res.domain like ?",domain).Find(&res)
	if err != nil {
		log.Error(err.Error())
		return res, err
	}
 ...
 func DeteleBindByMenuId(mid int64) error {
 	_, err := Orm.Exec("delete from menu_res where menu_id = ?", mid)
	 if err != nil {
		 log.Error(err.Error())
		 return  err
	 }
	 Orm.ClearCache(new(MenuRes))
 	return nil
 }
