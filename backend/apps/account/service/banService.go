...
func BanErrIp(ip string) error {
	...
	if errtimes >= settings.VOptions.GetInt64("NumVal.MAX_LOGIN_ERR_IP_TIMES") {
		return commons.ProduceUserBanOrRelease(accUtils.BAN_TYPE_IPADDRESS, "ban", ip)
	}
...
func ReleaseErrIp(ip string) error {
	...
	return commons.ProduceUserBanOrRelease(accUtils.BAN_TYPE_IPADDRESS, "release", ip)
}
