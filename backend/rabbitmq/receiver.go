...
func (mq *RabbitMQ) sendMsg(body string) {
	...
	if mq.ExchangeName != "" {
		...
		err = ch.ExchangeDeclare(mq.ExchangeName, mq.ExchangeType, false, false, false, false, nil)
		if err != nil {
	...
	_, err = ch.QueueDeclare(mq.QueueName, false, false, false, false, nil)
	if err != nil {
...
func (mq *RabbitMQ) sendRetryMsg(body string, retry_nums int32, args ...string) {
	...
	//fmt.Printf("%+v",mq)
	// 用于检查队列是否存在,已经存在不需要重复声明
	_, err = ch.QueueDeclare(mq.QueueName, false, false, false, false, table)
	if err != nil {
		log.Printf("QueueDeclare err :%s \n", err)
	}
	// 绑定任务
